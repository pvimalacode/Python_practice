...
def get_eng():
    ...
    for i in range(12):
        ...
        energy.append(x)
        #print(x)
    energy1=[item if item else 'null' for item in energy]
    #return([int(item) if item.isdigit() else 0 for item in energy ])
    y=[int(item) if item.isdigit() else 0 for item in energy]   #removing non numeric values
    return([x if x!=0 else sum(y)/len(y) for x in y ],energy1)  # replacing with mean
...
x,y=get_eng() # get X column values
...
while counter < 12:
    print
    print ('%7s  %4d\t%2d\t%s\t%d\t%s\t\t%s\t  %s' %(months[counter],bill1[counter], bill2[counter], y[counter],savings[counter],max_sav[counter],min_sav[counter],avg_sav[counter])) 
    counter = counter + 1
